...
// JWTChatHandler handles chat completion requests with JWT authentication.
// This is for the frontend Chat page, where users chat directly after login.
type JWTChatHandler struct {
	chatSvc *service.ChatService
}

// NewJWTChatHandler creates a new JWT chat handler.
func NewJWTChatHandler(chatSvc *service.ChatService) *JWTChatHandler {
	return &JWTChatHandler{chatSvc: chatSvc}
}

// HandleChat handles POST /api/chat/completions (JWT authenticated)
// This endpoint uses the same OpenAI-compatible format but authenticates via JWT
// instead of API Key, allowing logged-in users to chat without creating API keys.
func (h *JWTChatHandler) HandleChat(c *fiber.Ctx) error {
	...
	// Get tenant context from JWT middleware
...
func (h *JWTChatHandler) handleNonStream(c *fiber.Ctx, tc *middleware.TenantContext, req *openai.ChatCompletionRequest) error {
...
func (h *JWTChatHandler) handleStream(c *fiber.Ctx, tc *middleware.TenantContext, req *openai.ChatCompletionRequest) error {
	start := time.Now()
